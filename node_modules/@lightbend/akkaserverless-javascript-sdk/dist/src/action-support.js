...
module.exports = class ActionServices {
    ...
    addService(component, allComponents) {
        this.services[component.serviceName] = new ActionSupport(component.root, component.service, component.commandHandlers, allComponents);
    }
    componentType() {
    ...
    createHandler(call, callback, data) {
        ...
        if (service && service.service.methods.hasOwnProperty(data.name)) {
            if (service.commandHandlers.hasOwnProperty(data.name)) {
                ...
                this.reportError('Service call ' +
                    data.serviceName +
                    '.' +
                    data.name +
                    ' not implemented', call, callback);
            }
        }
        else {
            this.reportError('No service call named ' +
                data.serviceName +
                '.' +
                data.name +
                ' found', call, callback);
